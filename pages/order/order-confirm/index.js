...
Page({
  ...
  onSureCommit() {
    ...
    if ((outOfStockGoodsList && outOfStockGoodsList.length > 0) || (inValidGoodsList && storeGoodsList)) {
      ...
      const safeOut = Array.isArray(filterOutGoodsList) ? filterOutGoodsList : [];
      const safeStore = Array.isArray(filterStoreGoodsList) ? filterStoreGoodsList : [];
      const goodsRequestList = safeOut.concat(safeStore);
      this.handleOptionsParams({ goodsRequestList });
    }
  },
  // 提交订单
