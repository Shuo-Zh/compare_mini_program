...
export function genGood(id, available = 1) {
  ...
  const idStr = `${id}`;
  if (specID.indexOf(idStr) > -1) {
    return allGoods.filter((good) => good.spuId === idStr)[0];
  }
  const index = id % allGoods.length;
  const item = allGoods[index] || allGoods[0]; // 确保item不为undefined
  return {
    ...item,
    spuId: idStr,
    available: available,
    ...
    primaryImage: item?.primaryImage || 'https://tdesign.gtimg.com/miniprogram/template/retail/goods/nz-09a.png', // 确保primaryImage存在
  };
}
